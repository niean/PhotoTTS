...
根据 task_type 查 AGENTS.md "上下文管理 > 任务类型加载矩阵"，完整读取对应"必读"列的文件：

| task_type | 必读（完整读取） |
|-----------|----------------|
| feature | 01-overview, 02-architecture, 22-file-map, 01-prd-sense, 02-prd-baseline |
| bugfix | 01-overview, 03-conventions, 22-file-map |
| governance | 01-overview, 03-conventions, 22-file-map |
| docs | 01-overview, 22-file-map |

"按需读取"列的文件不在此步骤加载，由后续 Phase 根据需要自行读取。

加载矩阵的权威定义在 AGENTS.md，本表为快速参考；冲突时以 AGENTS.md 为准。
