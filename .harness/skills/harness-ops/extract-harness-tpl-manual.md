...
5. Subskills 保留通用维度的 scan-*.md（如架构、编码、安全、日志、废弃代码），剥离项目专属信息、替换为 `{{占位符}}`，剔除仅项目专属维度（如 scan-image-handling.md）；引用 subskills 的文件（workflows/harness-ops/governance-code-manual.md、skills/harness/verify-acceptance.md）同步蒸馏，项目专属维度的引用替换为占位符，通用维度的引用保留
