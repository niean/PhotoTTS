...
触发：人工指令，或由 Skill: 治理全部 Step 2 调用。对 .harness/agents/、skills/、subagents/ 进行扫描、清理与提取。
...
### Step 1 -- 读取现状
- Agent: Orchestrator
- 读取 AGENTS.md 注册表 + .harness/agents/、skills/、subagents/ 全部文件

### Step 2 -- 扫描与修复
- Agent: Reviewer（扫描） -> 用户确认 -> Coder（修复）
- 检查：注册表与实际文件不一致、格式不规范、闲置 Agent/Subagent、引用不存在的实体、目录枚举不同步、引用方向违反 AGENTS.md 规则（反向引用上层、使用绝对路径）

### Step 3 -- 清理与合并
- Agent: Reviewer（识别） -> 用户确认 -> Coder（执行）
- 识别：功能重叠可合并的 Agents/Skills/Subagents、已被覆盖或长期未用的能力
...
- 回顾 `git log --oneline -20`，识别 Agent/Skill/Subagent 候选（职责拆分、可复用流程、可并行化检查）
