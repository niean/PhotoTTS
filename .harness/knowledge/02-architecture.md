...
- 能力层（PhotoTTS/Sources/Core/Handlers+Managers）：OCRService、TTSService、LLMService、NetworkService、SettingsManager、SessionRecordManager、PlayHistoryManager、MakeHistoryManager、DebugLogManager、BackgroundMakeManager、PerformanceMonitorManager、PeerTransferManager。PlayHistoryManager/MakeHistoryManager 将数据存储委托 SessionRecordManager（会话级 history.json）。BackgroundMakeManager 管理单个后台 MakeTask（持有独立 Coordinator）。PerformanceMonitorManager 负责实时监控 CPU、内存、磁盘、网络指标。PeerTransferManager 封装 MultipeerConnectivity 实现设备间直传（WiFi/蓝牙自动切换）
