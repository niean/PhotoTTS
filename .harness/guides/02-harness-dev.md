...
2. 提交需求、给AI
    - 推荐方式：引用`03-prd-specs.md`需求字段、到Chat消息框。弱模不遵守流程，可在Chat指令后追加要求：`请严格按照 迭代步骤 执行，禁止擅自修改跳过，特别是 意图确认、结果验收(如果有)`
...
建议：上下文污染影响Coding效果 => 不同任务，使用不同Chat窗口；差异甚大的任务，更要避免复用窗口。
