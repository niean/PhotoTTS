...
- Phase 2 需求探索与设计：按 superpowers brainstorming 流程执行
- Phase 3 实现计划：`[GATE]` 必须使用 `ask_followup_question` 等待用户确认后结束回复，禁止同一回复内继续 Phase 4
- Phase 4 代码实现：`[GATE-ENTRY]` 必须确认用户已在上一条消息中明确回复，按 superpowers executing-plans 流程执行
