...
读取 .harness/PROJECT.md "扫描维度" 表，使用 `.harness/framework/skills/harness/subskills/scan-dimension.md` 通用模板，根据变更文件类型匹配维度：UI 文件 -> 维度1/2/5；Core 文件 -> 维度1/2/3/5；图片处理相关 -> 追加维度4；涉及文件删除 -> 追加维度6。subagent 并行扫描（传入 dimension 和 rule_sources），每个维度独立输出结论。超 5 个维度分批执行。

### Step 3: 验收检查

对照 spec_criteria 逐项验证，输出每项通过/不通过。

如果调用方已通过 subagent spec review（如 subagent-driven-development 的 spec compliance reviewer），可跳过逐项 spec 合规检查，仅验证：(1) 全部 Blocking 级验收标准，(2) 跨 task 集成点。
